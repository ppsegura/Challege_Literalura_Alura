...
public class Principal{
    ...
    private void listarLibros() {
        libros = new HashSet<>(ILibroRepository.findAll());

        // Inicializa las colecciones de cada libro
        libros.forEach(libro -> {
            ILibroRepository.findByIdWithAutoresAndLenguajes(libro.getId()).ifPresent(libroConAutores -> {
    ...
    private void listarAutores() {
        List<Autor> autores = IAutorRepository.findAllWithLibros();
        if (autores.isEmpty()) {
    ...
    private void autorAnoConcreto() {
        ...
        scanner.nextLine();
        List<Autor> autors = IAutorRepository.findByFechaDeNacimientoLessThanEqualAndFechaDeFallecimientoGreaterThanEqual(anio);
        if (autors.isEmpty()) {
    ...
    private void librosPorIdioma() {
        ...
        var idiomaInput = scanner.nextLine().toLowerCase();

        try {
            Idioma idioma = Idioma.fromString(idiomaInput);
            List<Libro> libros = ILibroRepository.findByIdioma(idioma);
